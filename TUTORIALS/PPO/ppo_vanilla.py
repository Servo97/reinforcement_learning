...
class PPO:
    def __init__(self, policy_nw, env, hyperparams):
        ...
        self.actor = policy_nw(self.n_obs, self.n_actions, hyperparams['hidden_dim'])
        self.critic = policy_nw(self.n_obs, 1, hyperparams['hidden_dim'])
    ...
    def collect_batch_experiences(self):
        ...
        while t < self.timesteps_per_batch:
            ...
            for i in range(self.max_timesteps_per_ep):
                ...
                obs, reward, terminate, truncate, _ = self.env.step(action)
        ...
        b_obs = torch.tensor(b_obs, dtype=torch.float)
        b_actions = torch.tensor(b_actions, dtype=torch.float)
        b_log_probs = torch.tensor(b_log_probs, dtype=torch.float)
    ...
    def batch_returns(self, b_rewards):
        ...
        b_returns = torch.tensor(b_returns[::-1], dtype=torch.float)
        return b_returns
    ...
    def evaluate_critic(self, obss, actions):
        value = self.critic(obss).squeeze()
        mean = self.actor(obss)
        dist = torch.distributions.MultivariateNormal(mean, self.covariance_mat)
...
def train(env, hyperparams, actor_model, critic_model):
    ...
    model.train(timesteps=20_000_000)
